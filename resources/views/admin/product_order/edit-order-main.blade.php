...
<div class="content-wrapper">
        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-12 text-center">
                    <h1>Edit Sales Order (#{{ $data->sku }})</h1>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <form action="{{ route('admin.product_order.updateOrderMain', $data->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="order_type" id="order_type_domestic" value="domestic" {{ $data->order_type == 'domestic' ? 'checked' : '' }}>
                                <label class="form-check-label" for="order_type_domestic">Domestic</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="order_type" id="order_type_corporate" value="corporate" {{ $data->order_type == 'corporate' ? 'checked' : '' }}>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" id="ex_delivery_date" class="form-control" placeholder="Select Expected Delivery Date" value="{{ \Carbon\Carbon::parse($data->expected_delivery_date)->format('d M Y') }}">
                        ...
                        <div class="col-md-4">
                            ...
                            @if($data->corporate_order_file)
                                <div id="existingFileBox" class="mt-2 p-2 border rounded bg-light">
                                    <strong>Current File:</strong>
                                    <span>{{ $data->corporate_order_file }}</span>
                                    <a href="{{ asset('assets/products/'.$data->corporate_order_file) }}"
                                       target="_blank"
                    ...
                    <div class="row product-row">
                        ...
                        <div class="col-md-3">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <a href="{{ route('admin.master.production-goods.create') }}"
                                target="_blank"
                                class="text-primary"
                        ...
                        <div class="col-md-2">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <a href="{{ route('admin.master.size-measurement.create') }}"
                                target="_blank"
                                class="text-primary"
                                style="font-size:13px;">
                                    Create New +
                        ...
                        <div class="col-md-2">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <a href="{{ route('admin.master.colors.create') }}"
                                target="_blank"
                                class="text-primary"
                                style="font-size:13px;">
                                    Create New +
                            ...
                            <tr class="product-row-item">
                                ...
                                <td>
                                    <select name="designList[]" class="form-control form-control-sm row-design-select select2" required>
                                        <option value="{{ $set->production_goods_id }}" selected>{{ $set->design_number }}</option>
                                    </select>
                                </td>
                                <td>
                                    <select name="sizeList[]" class="form-control form-control-sm row-size-select select2" required>
                                        <option value="{{ $set->set_size }}" data-pcs="{{ $set->no_of_pcs }}" selected>{{ $set->size_measurement->name ?? '' }}</option>
                                    </select>
                                </td>
                                <td>
                                ...
                                <td>
                                    <input type="number" name="product_quantity[]" class="form-control form-control-sm row-qty-input" value="{{ $set->set_quantity }}" min="1">
                                </td>
                                <td>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content position-relative">
            <button type="button"
                    ...
                    style="top:10px; right:10px; font-size:30px;"
                    data-dismiss="modal">
                &times;
            </button>
...
.open-label{cursor:pointer;color:#007bff;font-weight:600;text-decoration:underline}
.modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);justify-content:center;align-items:center;z-index:9999}
.modal-content{background:#fff;width:420px;padding:20px;border-radius:8px}
.size-row{display:flex;justify-content:space-between;padding:6px;background:#eef2f7;margin-top:6px}
.counter button{width:28px;height:28px;border:none;background:#28a745;color:#fff}
</style>
...
flatpickr("#ex_delivery_date", {
    ...
    position: "auto left",
    static: false,
    defaultDate: "{{ \Carbon\Carbon::parse($data->expected_delivery_date)->format('Y-m-d') }}",
...
function loadSalesOrderMasterData() {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            // Populate existing rows in the table
            populateAllRowSelects();
            $('#productList .select2').select2();
        }
    });
}
