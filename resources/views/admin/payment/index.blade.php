...
@section('content')
    @if($isOwner ?? false)
        ...
        <div class="owner-container">
            ...
            <div class="{{ ($isOwner ?? false) ? 'app-filter-drawer' : 'card shadow-sm border-0 mb-3' }}" id="paymentFilterDrawer">
                @if($isOwner ?? false)
                                    ...
                                    <form method="GET" action="{{ route(($routePrefix ?? 'admin.payment.history.') . 'index') }}" class="w-100">
                                        <div class="row align-items-end">
                                            ...
                                            <div class="col-md-2 mb-2">
                                                ...
                                                <select class="form-control form-control-sm" name="payment_category">
                                                    <option value="">All Categories</option>
                                                    @foreach($categories as $category)
                                                        <option value="{{ $category }}" {{ request('payment_category') == $category ? 'selected' : '' }}>
                                                            {{ ucfirst(str_replace('_', ' ', $category)) }}
