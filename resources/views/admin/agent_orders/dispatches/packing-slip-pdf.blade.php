...
<head>
    <meta charset="utf-8">
    <title>Packing Slip - {{ $dispatch->id }}</title>
    <style>
        @page { margin: 0; }
        body { font-family: 'Helvetica', 'Arial', sans-serif; font-size: 11px; color: #222; margin: 0; padding: 30px; line-height: 1.4; }
        .main-container { border: 2px solid #000; padding: 0; margin: 0; min-height: 980px; position: relative; }
        ...
        <table class="items-table" cellpadding="0" cellspacing="0">
            ...
            <tbody>
                ...
                @foreach($groupedItems as $index => $item)
                    ...
                    @php $tP += $item->total_qty; $tB += $item->box_count; @endphp
                @endforeach
                ...
                <tr class="total-row">
                    ...
                    <td class="text-right" style="font-size: 13px;">{{ number_format($tP, 2) }} Pcs.</td>
                    <td class="text-right" style="font-size: 13px;">{{ number_format($tB, 2) }}</td>
