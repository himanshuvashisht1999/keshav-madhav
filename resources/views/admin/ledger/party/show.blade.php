...
<style>
    ...
    .text-debit { color: #d32f2f; font-weight: 600; }
    .text-credit { color: #2e7d32; font-weight: 600; }
    .text-balance { font-weight: 700; color: #1e3c72; }
    .date-badge {
        background: #f1f3f5;
        color: #495057;
        padding: 4px 10px;
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-md-7 text-right">
                        <div class="d-flex justify-content-end align-items-center">
                            <div class="bg-white text-dark p-3 px-4 rounded shadow-sm mr-3 text-center border-left border-secondary" style="min-width: 180px; border-left-width: 4px !important;">
                                ...
                                <h4 class="mb-0 font-weight-bold">
                                    ...
                                    <span class="badge {{ $openingBalAmount >= 0 ? 'badge-info' : 'badge-warning' }} ml-1" style="font-size: 11px;">{{ $openingBalAmount >= 0 ? 'DR' : 'CR' }}</span>
                                </h4>
                            </div>
                            <div class="bg-white text-dark p-3 px-4 rounded shadow-sm text-center border-left border-primary" style="min-width: 180px; border-left-width: 4px !important;">
                                ...
                                <h4 class="mb-0 font-weight-bold text-primary">
                                    ...
                                    <span class="badge {{ (($type === 'customer' && $party->balance <= 0) || ($type === 'vendor' && $party->balance >= 0)) ? 'badge-success' : 'badge-danger' }} ml-1" style="font-size: 11px;">
                                        @if($type === 'customer')
                    ...
                    <form action="{{ route('admin.ledger.party.show', ['type' => $type, 'id' => $party->id]) }}" method="GET">
                        <div class="row align-items-end">
                            ...
                            <div class="col-md-4">
                                ...
                                <a href="{{ route('admin.ledger.party.show', ['type' => $type, 'id' => $party->id]) }}" class="btn btn-outline-secondary ml-2" style="border-radius: 10px;">Clear</a>
                        ...
                        <table class="table table-bordered table-ledger mb-0">
                            ...
                            <tfoot class="bg-light">
                                <tr class="font-weight-bold">
                                    ...
                                    <td class="text-right py-3 text-debit">₹ {{ number_format($transactions->sum('debit'), 2) }}</td>
                                    <td class="text-right py-3 text-credit">₹ {{ number_format($transactions->sum('credit'), 2) }}</td>
