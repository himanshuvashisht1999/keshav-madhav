...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-8">
                    <h3 style="font-size:1.2rem;">Item Stock SKU : <span class="text-muted">{{ $items->sku }}</span></h3>
                </div>
                <div class="col-sm-4">
            ...
            <div class="card card-default ">
                 <!-- <div class="row" >
                    <div class="col-12 card-header">
                        <h3 class="card-title">Manage Item Stock</h3>
                    </div>
                </div> -->
                ...
                <!-- <div class="d-flex justify-content-end me-3 ">
                    ...
                    <button name="item_quantity" class="btn btn-primary m-1 " onclick="window.location.href='{{ route('admin.item_stock.itemQuantityExcel') }}'">
                        <i class="nav-icon fas fa-download"></i> Item Quantity
                    </button>
                </div> -->
                ...
                <table id="customers" class="table table-bordered table-hover">
                  <thead>
                    <tr role="row" class="filter">
                        ...
                        <td>
                           <input type="hidden" class="form-control" name="sku" id="sku" value="{{$items->sku}}" autocomplete="off">
                        </td>
...
<script>
    $(function () {
        ...
        var oTable = $('#customers').DataTable({
            ...
            "pageLength":25,
            ajax: {
                url: '{!! route('admin.item_stock.indexList') !!}',
                data: function (d) {
                    d.id = $('#id').val();
                    d.sku = $('#sku').val();
                    d.date = $('#date').val();
            ...
            buttons: [
                ...
                {
                    text: `<i class="nav-icon fas fa-download"></i> Item Quantity`,
                    attr: {
                        id: 'item_quantity',
                        name: 'item_quantity'
                    },
                    className: 'btn-datatable',
                    action: function (e, dt, node, config) {
                        window.location.href = "{{ route('admin.item_stock.itemQuantityExcel') }}";
                    }
                }
            ]
        });
    ...
    $(document).ready(function () {
        $('#generatePDF').on('click', function (e) {
            ...
            var form = $('<form>', {
                action: '{{ route("admin.item_stock.generatePdf") }}',
                method: 'POST',
                target: '_blank'
        ...
        $('#generateExcel').on('click', function (e) {
            ...
            var form = $('<form>', {
                action: '{{ route("admin.item_stock.generateExcel") }}',
                method: 'POST',
                target: '_blank'
            }).append($.map(data, function(v, k) {
                return $('<input>', { type: 'hidden', name: k, value: v });
