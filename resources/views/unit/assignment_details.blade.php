...
@section('header_right')
    ...
    @elseif(isset($transaction->production_slip_digitization_id))
        <a href="{{ route('unit.download.slip', ['id' => $transaction->production_slip_digitization_id]) }}" style="color: white; font-size: 20px;">
            <i class="fas fa-file-download"></i>
        </a>
    @endif
@endsection
...
@section('content')
        ...
        <div class="card">
            ...
            @if($isRework)
                <div style="position: absolute; top: 0; left: 0; padding: 8px 16px; background: #e11d48; color: white; font-weight: 800; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; border-bottom-right-radius: 20px;">
            ...
            @if($header['remark'] && $header['remark'] != '-')
                <div style="margin-top: 15px; padding: 12px; background: #fffbe6; border-radius: 12px; border: 1px solid #ffe58f; font-size: 13px; color: #856404;">
        ...
        <div class="card" style="padding-bottom: 5px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-bottom: 20px;">
                <div style="background: #fdf2f8; border-radius: 16px; padding: 15px; border: 1px solid #fce7f3; text-align: center;">
                    <span style="font-size: 10px; color: #be185d; text-transform: uppercase; font-weight: 800; display: block; margin-bottom: 5px; letter-spacing: 0.5px;">Master Set</span>
                    <span style="font-size: 18px; color: #831843; font-weight: 900;">{{ $header['size_set'] ?? '-' }}</span>
                </div>
                <div style="background: #f0f9ff; border-radius: 16px; padding: 15px; border: 1px solid #e0f2fe; text-align: center;">
                    <span style="font-size: 10px; color: #0369a1; text-transform: uppercase; font-weight: 800; display: block; margin-bottom: 5px; letter-spacing: 0.5px;">Pcs/Set</span>
                    <span style="font-size: 18px; color: #0c4a6e; font-weight: 900;">{{ $header['pcs_in_set'] ?? 0 }}</span>
                </div>
                <div style="background: #f0fdf4; border-radius: 16px; padding: 15px; border: 1px solid #dcfce7; text-align: center;">
                    <span style="font-size: 10px; color: #15803d; text-transform: uppercase; font-weight: 800; display: block; margin-bottom: 5px; letter-spacing: 0.5px;">Grand Total</span>
                    <span style="font-size: 18px; color: #064e3b; font-weight: 900;">{{ number_format($header['total_pcs'], 0) }}</span>
                </div>
            </div>
            ...
            <div style="padding: 0 5px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; border-bottom: 1px solid #f1f5f9; padding-bottom: 8px;">
                     ...
                     <span style="font-size: 11px; background: #3b82f6; color: white; padding: 2px 8px; border-radius: 10px; font-weight: 600;">{{ count($sizeData) }} Sizes</span>
                </div>
                    ...
                    <tbody>
                        ...
                        @foreach($sizeData as $row)
                            ...
                            <tr>
                                ...
                                <td style="text-align: right; font-weight: 800; color: #1e293b; padding-right: 5px;">{{ number_format($row['pcs'], 0) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr style="background: #f8fafc; border-top: 2px solid #e2e8f0;">
                            ...
                            <td style="text-align: right; padding: 15px 12px; color: #2563eb; font-size: 18px; font-weight: 900;">
            ...
            <form action="{{ route('unit.submit') }}" method="POST" id="uploadForm">
                ...
                @if($type === 'cutting')
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
                        <div>
                            <label style="font-size: 11px; font-weight: 700; color: #64748b; text-transform: uppercase;">Next Stage</label>
                            <select name="to_stage_id" class="btn-action" style="width: 100%; background: white; color: #1e293b; border: 1.5px solid #cbd5e1; text-align: left; padding: 10px;" required>
                        ...
                        <div>
                            <label style="font-size: 11px; font-weight: 700; color: #64748b; text-transform: uppercase;">Assign Lot No</label>
                            <input type="text" name="lot_no" class="btn-action" style="width: 100%; background: white; color: #1e293b; border: 1.5px solid #cbd5e1; text-align: left; padding: 10px;" placeholder="e.g. 601" required>
                        </div>
                    </div>
                @endif
