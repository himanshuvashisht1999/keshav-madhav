...
use App\Http\DataTable\Admin\Master\FabricWeaveDataTable as DataTable;

class FabricWeaveService {
    public function __construct(
        DataTable $datatable,
        FabricWeave $fabric_weave
    ) {
        $this->datatable= $datatable;
        $this->fabric_weave= $fabric_weave;
    }
    ...
    public function edit(Request $request){
        $data = FabricWeave::where('id',$request->id)->first();
        return $data;
    }
    public function update(Request $request){
        $update_data = FabricWeave::find($request->id);
        // if($request->file('image')){
    ...
    public function delete(Request $request){
        $data = FabricWeave::where('id',$request->id)->update([
            'status' => 0,
        ]);
        return $data;
    }
