...
use App\Http\DataTable\Admin\Master\MasterProductTypesDataTable as DataTable;

class MasterProductTypesService {
    public function __construct(
        DataTable $datatable,
        MasterProductType $product_types
    ) {
    ...
    public function store(Request $request){
        ...
        // }
        $save_data = new MasterProductType;
    ...
    public function edit(Request $request){
        $data = MasterProductType::where('id',$request->id)->first();
        return $data;
    }
    public function update(Request $request){
        $update_data = MasterProductType::find($request->id);
        // if($request->file('image')){
    ...
    public function delete(Request $request){
        $data = MasterProductType::where('id',$request->id)->update([
            'status' => 0,
        ]);
        return $data;
    }
