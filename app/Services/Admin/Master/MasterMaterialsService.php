...
use App\Http\DataTable\Admin\Master\MasterMaterialsDataTable as DataTable;

class MasterMaterialsService {
    ...
    public function edit(Request $request){
        $data = MasterMaterial::where('id',$request->id)->first();
        return $data;
    }
    public function update(Request $request){
        $update_data = MasterMaterial::find($request->id);
        // if($request->file('image')){
    ...
    public function delete(Request $request){
        $data = MasterMaterial::where('id',$request->id)->update([
            'status' => 0,
        ]);
        return $data;
    }
