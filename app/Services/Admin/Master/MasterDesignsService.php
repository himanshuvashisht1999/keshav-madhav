...
use App\Http\DataTable\Admin\Master\MasterDesignsDataTable as DataTable;

class MasterDesignsService {
    ...
    public function edit(Request $request){
        $data = MasterDesign::where('id',$request->id)->first();
        return $data;
    }
    public function update(Request $request){
        $update_data = MasterDesign::find($request->id);
        // if($request->file('image')){
    ...
    public function delete(Request $request){
        $data = MasterDesign::where('id',$request->id)->update([
            'status' => 0,
        ]);
        return $data;
    }
