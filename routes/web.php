...
Route::group(['prefix' => 'admin', 'as' => 'admin.', 'middleware' => ['web']], function () {
    ...
    Route::middleware(['checkAdminLogin'])->group(function () {
        ...
        Route::prefix('/order-dispatch')->name('order-dispatch.')->group(function () {
            ...
            Route::get('/delete', [AdminOrderDispatchController::class, 'delete'])->name('delete');
        });
        ...
        Route::prefix('/inventory')->name('inventory.')->group(function () {
            ...
            Route::get('/get-variant-images/{id}', [AdminInventoryController::class, 'getVariantImages'])->name('get_variant_images');
    ...
    Route::middleware(['checkAdminLogin'])->group(function () {
        ...
        Route::prefix('payment')->name('payment.')->group(function () {
            ...
            Route::prefix('journal-voucher')->name('journal-voucher.')->group(function () {
                ...
                Route::post('/delete/{id}', [AdminJournalVoucherController::class, 'destroy'])->name('delete');
            });
        });
...
Route::get('/run-today-database-updates', function (\Illuminate\Http\Request $request) {
    try {
        set_time_limit(300);
        $log = [];

        // 1. Run Migrations with --force for production
        try {
            \Illuminate\Support\Facades\Artisan::call('migrate', [
                '--path' => 'database/migrations/2026_09_12_160000_backfill_inbound_sessions_history.php',
                '--force' => true,
            ]);
            $log['migration_backfill_history'] = trim(\Illuminate\Support\Facades\Artisan::output());
        } catch (\Throwable $e) {
            $log['migration_backfill_history_note'] = $e->getMessage();
        }

        try {
            \Illuminate\Support\Facades\Artisan::call('migrate', [
                '--path' => 'database/migrations/2026_09_12_170000_reconcile_consumed_stock_total_boxes.php',
                '--force' => true,
            ]);
            $log['migration_reconcile_boxes'] = trim(\Illuminate\Support\Facades\Artisan::output());
        } catch (\Throwable $e) {
            $log['migration_reconcile_boxes_note'] = $e->getMessage();
        }

        // 2. Direct & Semantic Update of Domestic Inventory Total Boxes (Idempotent safety)
        $directUpdates = [
            6642 => 1, 6643 => 1, 6644 => 2, 6645 => 2,
            6630 => 1, 6631 => 1, 6632 => 1, 6633 => 1,
            6634 => 1, 6635 => 1, 6636 => 1, 6637 => 1,
            6638 => 1, 6639 => 1, 6640 => 1, 6641 => 1,
            6627 => 1, 6628 => 1, 6629 => 1,
            6626 => 1, 6624 => 2, 6597 => 2,
            6589 => 1, 6592 => 1, 6576 => 1, 6577 => 1,
            6572 => 1, 6573 => 1, 6574 => 3,
            6562 => 1, 6563 => 1, 6560 => 1, 6561 => 1,
            6506 => 1, 6505 => 1,
            6499 => 1, 6500 => 1, 6501 => 1, 6502 => 1, 6503 => 1, 6504 => 1,
        ];

        $directUpdatedCount = 0;
        foreach ($directUpdates as $id => $targetBoxes) {
            $updated = \Illuminate\Support\Facades\DB::table('domestic_inventories')
                ->where('id', $id)
                ->where('total_boxes', '<', $targetBoxes)
                ->update(['total_boxes' => $targetBoxes]);
            if ($updated) $directUpdatedCount += $updated;
        }
        ...
        foreach ($semanticItems as $item) {
            ...
            if ($updated) $semanticUpdatedCount += $updated;
        }

        $log['inventory_direct_updated_rows'] = $directUpdatedCount;
        $log['inventory_semantic_updated_rows'] = $semanticUpdatedCount;

        // 3. Run Inbound History Backfill Command (supports optional session filter e.g. ?session=1132)
        try {
            $backfillParams = [];
            if ($request->filled('session')) {
                $backfillParams['--session'] = $request->query('session');
            }
            \Illuminate\Support\Facades\Artisan::call('inventory:backfill-inbound-sessions', $backfillParams);
            $log['backfill_inbound_history_output'] = trim(\Illuminate\Support\Facades\Artisan::output());
        } catch (\Throwable $e) {
            $log['backfill_inbound_history_note'] = $e->getMessage();
        }
        ...
        $log['caches_cleared'] = true;

        return response()->json([
            'status' => 'success',
            'message' => 'Today\'s database updates and stock reconciliations have been completed successfully.',
            'details' => $log
        ]);
    } catch (\Throwable $e) {
        return response()->json([
